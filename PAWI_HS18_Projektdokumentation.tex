...
\begin{figure}[h!]
  ...
  \caption{Vereinfachte Layer Architekturübersicht}
\end{figure}
...
Die HoloLens zeigte grosse Schwächen beim Gebrauch im Freien, da deren Bildschirm für den Aussengebrauch zu wenig Hell ist. Zudem fehlen der HoloLens Lokalisierungssensoren. Im Rahmen einer Analyse der Bedienung, wurden 5 Probanden dazu Aufgefordert mit der vorhanden HoloLens App zu Interagieren dabei wurde ihr Verhalten beobachtet. Die Probanden besassen keinerlei Erfahrung im Umgang mit der HoloLens und es wurde ihnen auch keine vorgängigen Informationen zur Bedienung mitgeteilt. Bei diesem Test stellte sich heraus, dass alle Probanden intuitiv mit dem Objekt falsch Interagieren wollten.
Nach einer kurzen Aufklärung konnten jedoch alle die Applikation ohne grössere Probleme bedienen.
...
Während der Recherchen stellte sich heraus, dass es ein breites Spektrum an verschiedenen AR Frameworks auf dem Markt vorhanden sind. Jedes Framework setzt dabei auf eigene Anwendungsbereiche. Für dieses Projekt wurden 4 häufig verwendete und gut dokumentierte Frameworks näher untersucht (\cite{DDIDevelopment}). Konkret handelte es sich dabei um folgende Frameworks:
\begin{itemize}
\item Kudan
\item Vuforia
\item Wikitude
...
Für das Projekt wurde entschieden Kudan als AR Framework zu verwenden. Gründe dafür liegen darin, dass dieses Framework über ein sehr robuste single-Kamera SLAM Implementation verfügt (\cite{BerfinAyhan2017}). Weiterer Grund lag in der Lizenzgestaltung, welche eine Veröffentlichung ohne weitere Kosten ermöglicht. Der wichtigste Grund liegt darin, dass das Knowhow über Kudan am grössten war.

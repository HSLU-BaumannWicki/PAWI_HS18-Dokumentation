...
Hier werden die Anforderungen aufgelistet und ob diese Erfüllt wurden oder nicht. Sofern diese nicht erfüllt wurden, wird für dessen Erfüllung eine Potentielle Lösungsidee beschrieben.
\bigbreak
Legende:
...
Um die Anforderung von 1.1 erfüllen zu können, müsste die Nordinitialisierung erheblich verbessert werden, da dieser den Ungenauesten Sensor darstellt. Eine Potentielle lösung für dieses Problem kann die Kontinuirliche Berechnung von Norden mit den Bewegungen des Smartphones darstellen. Dies ist jedoch in der zur Verfügung stehender Zeit nicht Lösbar gewesen.

\subsection{Verbesserung für \#2}
Um die Anforderung von Nummer 2 erfüllen zu können, braucht es noch einen Indikator, welcher die Richtung des aussersichtweite geratenes Objekt anzeigt. Kudan liefert bereits die Möglichkeit die Position zum View point zu berechnen. Damit lässt sich die Position des Pfeiles bestimmen und er kann an der korrekten Position dargestellt werden. Die Implementation konnte jedoch aus Zeitlichen Gründen nicht Implementiert werden.
\subsection{Verbesserung für \#3}
Die Momentanige Architektur lässt es mit einfachen Anpassungen zu, dass die Activity selbst über Location Updates informiert wird. So müsste um diese Anforderung zu erfüllen die Activity das Interface LocationFilterListener implementieren und sich selber bei LocationFilter registrieren. So würde die Activity über alle LocationUpdates informiert werden. Um nun dem Benutzer die Information Anzuzeigen, müsste im Hintergrund ein Timer laufen, welcher nach 3 Sekunden eine Information an den Benutzer liefert. Bei eingegangene Location müsste der Timer wieder Zurückgesetzt werden.
Die Implementation konnte jedoch aus Zeitlichen Gründen nicht Implementiert werden. 

\section{Projekt Fazit}
Die Projektarbeit im Rahmen dieses Modules hat uns sehr viel gebracht. Wir konnten viel mitnehmen und wissen nun viel genauer, was uns bei der Bachelorarbeit erwartet. Die Zusammenarbeit und Kommunikation innerhalb des Teams verlief einwandfrei, auch die Kommunikation mit dem Experten war sehr gut und angenehm.
\bigbreak
Folgend kommen nun Verbesserungen, welche bei einem nächsten Projekt besser gelöst werden sollen.
Erster Punkt, welchen es zu verbessern gilt, ist mit der Dokumentation früher im Projekt zu beginnen.  Die Dokumentation ist nicht erst während der letzten
Wochen entstanden, aber es gab viele Punkte, die bis zum Schluss nicht ganz klar und somit noch nicht dokumentiert waren.
Eine weiterer Verbesserung wäre, dass wir uns schneller auf eine Plattform und ein Framework beschränken. Eine Eingrenzung der Plattform und des Framworkes hätte früher im Projekt eine Abgrenzung gesetzt und die Entwicklung des eigentlichen Prototypen fokussiert.
...
\bigbreak
Weiter gilt es zu beachten, dass auch positives aus diesem Projekt genommen werden kann. So konnten wir den Umgang mit den Hilfmittel stärker Vertiefen. Zudem konnte für die Dokumentation sowie den Code der Git Feature Branch Workflow etabliert werden, welche das Simultane arbeiten stark vereinfachte und eine Optimale Code und Dokumentationsüberprüfung des anderen Projektmitgliedes forcierte. Ein weiterer Vorteil ist zudem das nun Vorhandene wissen wie eine CI \& CD Infrastruktur für Code sowie Dokumentation aufgebaut werden kann.
...
\begin{itemize}
	...
	\item Verbesserung der Nordinitialisation
	\item Verbesserung der Höhenlogik mit OpenElevation
	\item Implementation einer GPS Lösung mit SLAM Implementierung (Somit kann die Darstellungsgenauigkeit verbessert werden).

...
\begin{figure}[h!]
	\center
	\includegraphics[keepaspectratio, width=0.85\textwidth]{ARShowBuilding_at_HSLU.png}
	\caption{Darstellung des Gebäudes in Rotkreuz}
...
Die Problematik dabei ist, dass wir also nur auf eine definiert Genauigkeit unsere Position bestimmen können. Wenn wir uns ein über den Globus gespanntes Raster mit der Distanz zwischen zwei Rasterlinien der Unterteilung eines Grads entsprechend, können wir über das Bogenmass die maximale Abweichung ausrechnen.
Wie der Tabelle entnommen werden kann, entspricht die maximale Abweichung von den wahren Koordinaten (angenommen die empfangenen Daten sind absolut korrekt) $0.06$ Meter (die Hälfte des Rasters). Um zusätzlich nicht von Rate Limiting blockiert zu werden (weil zuviele Abfragen in zu kurzer Zeit geschickt werden), vergrössern wir den Abstand zwischen Rasterlinien künstlich auf $10^{-4}$ Grad. Es wird also erst eine neue Abfrage generiert, wenn die Position einen solchen Unterschied zu der Position einer vorhergegangenen Abfrage besitzt. Dieser Abstand von $10^{-4}$ Grad entspricht einer Abweichung von elf Metern, die Annahme ist dabei, dass die Höhe innerhalb dieser Distanz nicht sprunghaft ansteigt oder sinkt.
...
\hline
	T-010v1 & Es soll überprüft werden, ob das bestehende Gebäude mit dem AR Gebäude deckungsgleich ist. & \xmark \\
\hline
